function start_meet_click() {
    // app.getRecipients(function(emails) {
        var emails = localStorage.getItem("emailsforMeet")
        var meet_options = {
            ...
            var intervalTimer = setInterval(function() {
                ...
                getRecipients(function(emails) {
                    ...
                    if (emails.length == 0) {
                        ...
                        $('#binderListul').empty();
                        $('#binderListul').append('<div style="width: 100%; margin-top: 80px; text-align: center; color: grey;"><h2>Start a New Conversation</h2><p>Click on the "New Conversation" button to start collaborating and communicating with your email recipients.</p></div>');

                        return;
                    }
                    localStorage.setItem("emailsforMeet",emails)
                    //$('#waitspin').show();
                        ...
                        if (data.data.binders == undefined || data.data.binders == null || data.data.binders.length == 0) {
                            ...
                            data.data.binders.forEach(function(binder) {
                                ...
                                $('#binderListul').append('<li class="list-group-item share-binder-item" onclick="openBinder(\'' + binder.binder.id + '\',false)">' +
                                    ...
                                    '</div>' +
                                    '<div class="media-body media-body-sm"><div class="binder-name" style="font-size:12px;max-width: 160px;">' + bindername + '</div></div>' +
